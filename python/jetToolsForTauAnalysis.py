#taken from /PhysicsTools/PatAlgos/python/tools/jetTools.py with the following replacements:
#process.patBeforeLevel0Reco --> process.patBeforeLayer0JetsForTauAnalyses
#process.patLayer0 --> process.patLayer0JetsForTauAnalyses
#process.patLayer1 --> process.patLayer1JetsForTauAnalyses
...
def switchJetCollection(process,jetCollection,layers=[0,1],runCleaner="CaloJet",doJTA=True,doBTagging=True,jetCorrLabel=None,doType1MET=True):
    ...
    if runCleaner == "CaloJet":
        ...
        process.patLayer0JetsForTauAnalyses.remove(process.allLayer0Jets)
        # MC match
    ...
    if doBTagging :
          ...
          process.patLayer0JetsForTauAnalyses.replace(process.patBeforeLayer0JetsForTauAnalyses, btagSeq + process.patBeforeLayer0JetsForTauAnalyses)
          ...
          if runCleaner != None:
              ...
              process.patLayer0JetsForTauAnalyses.remove(process.patAODJetTracksAssociator)
              process.patLayer0JetsForTauAnalyses.remove(process.patAODBTags)
              process.patLayer0JetsForTauAnalyses.remove(process.patAODTagInfos)
    else:
        process.patLayer0JetsForTauAnalyses.remove(process.patAODBTagging)
        process.patLayer0JetsForTauAnalyses.remove(process.patLayer0BTagging)
    ...
    if doJTA or doBTagging:
        if not doBTagging:
            ...
            process.patLayer0JetsForTauAnalyses.replace(process.process.patBeforeLayer0JetsForTauAnalyses, process.jetTracksAssociatorAtVertex + process.process.patBeforeLayer0JetsForTauAnalyses)
            process.patAODJetTracksAssociator.src       = jetCollection
    ...
    if jetCorrLabel != None:
        ...
        if runCleaner == None:
            ...
            process.patLayer0JetsForTauAnalyses.remove(process.jetCorrFactors)
    else:
        process.patLayer0JetsForTauAnalyses.remove(process.jetCorrFactors)
        process.patLayer0JetsForTauAnalyses.remove(process.layer0JetCorrFactors)
        if layers.count(1) != 0:
...
def addJetCollection(process,jetCollection,postfixLabel,
    ...
    def addAlso (label,value):
        ...
        process.patLayer0JetsForTauAnalyses.replace( existing, existing * value )
        if layers.count(1) != 0 : process.patLayer1JetsForTauAnalyses.replace( existing, existing * value )
    ...
    if doJTA or doBTagging:
        if not doBTagging:
            ...
            setattr( process, jtaLabel, ic5JetTracksAssociatorAtVertex.clone(jets = cms.InputTag(jetCollection)) )
            process.patLayer0JetsForTauAnalyses.replace(process.patBeforeLayer0JetsForTauAnalyses, getattr(process,jtaLabel) + process.patBeforeLayer0JetsForTauAnalyses)
            if runCleaner != None:
                addClone('patAODJetTracksAssociator', src=cms.InputTag(jetCollection), 
